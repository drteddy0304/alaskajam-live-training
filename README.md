...
- 提供されたCAT WALK LIFE.mp3の冒頭40秒を再生。ダミー譜面は実曲のテンポ・構成や公式Cheering Guideを再現しません。
...
- 音源を読み込んだ後、3秒カウントダウンで開始。MP3は約7.2MB。練習は冒頭40秒で終了。
...
3. `source.name / url / revision`に正本と版を記録。`kind`を`official`にする。各
otes`は時刻順の`{ "time": 12.5, "action": "CLAP" }`形式。WIPERには`"direction": "left"`または`"right"`を必須とする。
