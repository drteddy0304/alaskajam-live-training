...
- 音源を読み込んだ後、3秒カウントダウンで開始。MP3は約7.2MB。練習は224.39秒で終了。
- ログイン・DB・有料API・外部フォント・独自計測・ランタイム依存なし。お手本を開いたときのみYouTubeを読み込みます。動画側の広告等はYouTubeの仕様に従います。
...
- `audio.js`：音源の読み込み・デコード、音楽と判定の共通時計
...
5. 配信許諾を確認した音源だけを`assets/audio/`へ追加。`audio`を`{ "src": "./assets/audio/example.mp3", "offset": 0 }`へ変更。`offset`は譜面の0秒に相当する音源側の開始秒（0以上）。Web Audioの時計を音源と判定で共有し、停止・再開も連動する。
