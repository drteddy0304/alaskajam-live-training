...
- 8曲の一覧。CAT WALK LIFEのみ約3分44秒のフル尺ベータ。残り7曲は準備中。
- 提供されたCAT WALK LIFE.mp3を最後まで再生。公式Cheering Guideのクラップ・ワイパー・コールを参考にした暫定譜面です。正確な区間境界・コール時刻の転記は未完了で、公式動画と完全一致ではありません。
- 音あり練習は開始タップで曲を再生、1.8秒前から予告、タイミングバー、PERFECT / GOOD / MISS。
...
- 開始タップ内で標準の音声プレーヤーに再生を要求し、読み込みができ次第開始。スマホの再生制限に対応するため、音あり練習の3秒カウントダウンは省略。MP3は約7.2MB。練習は224.39秒で終了。
- ログイン・DB・有料API・外部フォント・独自計測・ランタイム依存なし。お手本は公式YouTubeへのリンクで開きます。
...
- `app.js`：表示、入力、停止・再開、保存
- `audio.js`：標準音声プレーヤーによる再生、再生位置と判定の同期
- `engine.js`：DOMに依存しない譜面検証と採点
- `data/songs.json`：一覧と譜面参照
- `data/cat-walk-life-full.json`：現在のフル尺ベータ譜面と出典情報
- `data/demo.json`：旧40秒ダミー譜面（テスト用）
- `GUIDE-NOTES.md`：動画確認箇所と暫定時刻の記録
...
3. `source.name / url / revision`に正本と版を記録。全動作と時刻を確認してから`kind`を`official`にする。各`notes`は時刻順の`{ "time": 12.5, "action": "CLAP" }`形式。WIPERには`"direction": "left"`または`"right"`を必須とする。
4. `duration`を秒数で指定し、最後のノートの0.28秒後以降にする。`leadTime`は予告秒数。現在のエンジンは単一アクション、隣接ノートの間隔は0.56秒超を必要とする。密な譜面・同時押しはエンジンの仕様変更とテストが必要。
5. 配信許諾を確認した音源だけを`assets/audio/`へ追加。`audio`を`{ "src": "./assets/audio/example.mp3", "offset": 0 }`へ変更。`offset`は譜面の0秒に相当する音源側の開始秒（0以上）。音声プレーヤーのcurrentTimeを判定時計に使い、停止・再開・読み込み待ちも連動する。
...
音源差し替え後は実機Safari / Chromeで同期・バックグラウンド停止・音源の読込失敗・再開を再確認してください。提供音源のデコードと出力、一時停止・再開、40秒終了、リトライ、音源読込失敗をブラウザで検証済みです。iPhone実機は未検証です。端末固有の入力遅延補正、オフラインキャッシュ、全曲譜面は未実装です。

### スマホ再生修正（mobile1）

AudioContextからHTMLAudioElementへ変更。play()は開始タップ内で、譜面取得などの非同期処理より前に呼び出す。対応端末ではAudio Sessionをplaybackに設定。30秒の読み込みタイムアウトを設け、再生拒否は開始画面へ戻して通知する。端末のメディア音量や出力先はアプリから変更できない。iPhone実機での出力確認はユーザーの再試行待ち。
